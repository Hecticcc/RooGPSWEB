...
import { MessageSquare, User, Smartphone, CreditCard, Hash } from 'lucide-react';
    ...
    <>
            ...
            <tr>
              ...
              <td>
                ...
                {(p?.mobile ?? data.email) && (
                  <>
                    {' '}
                    <button type="button" className="admin-btn" onClick={openSmsModal}>
                      <MessageSquare size={14} style={{ verticalAlign: 'middle', marginRight: 4 }} aria-hidden />
                      Send SMS
                    </button>
                  </>
                )}
      ...
      <div className="admin-card">
        ...
        <p className="admin-time" style={{ marginBottom: '0.75rem' }}>
          Price, next renewal (estimate), status. Suspend = disable the SIM for that order (use Disable SIM above).
        </p>
