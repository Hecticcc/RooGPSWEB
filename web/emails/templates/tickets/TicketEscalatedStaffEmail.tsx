...
export default function TicketEscalatedStaffEmail(p: TicketEmailPayload) {
  const ticketUrl = `${appBaseUrl}/admin/support/tickets/${p.ticketId}`;
  return (
    <EmailLayout preview={`Ticket #${p.ticketNumber} escalated`}>
      <Text style={heading}>Ticket escalated</Text>
      <Text style={paragraph}>Support ticket #{p.ticketNumber} has been escalated.</Text>
