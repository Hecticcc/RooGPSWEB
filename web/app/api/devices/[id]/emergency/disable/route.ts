...
export async function POST(
  ...
  const { data: device, error: devErr } = await admin
    .from('devices')
    .select('id, user_id, normal_profile, desired_mode, mode_transition_status')
    .eq('id', deviceId)
    .single();
  ...
  const normalProfile = (device as { normal_profile?: { gprs_interval_command_102?: string } | null }).normal_profile;
  if (!normalProfile?.gprs_interval_command_102) {
    return NextResponse.json(
      { error: 'Normal profile not saved. Enable Emergency Mode once to save it, then disable.' },
      { status: 400 }
    );
  }
  ...
  if (desired === 'NORMAL' && (status === 'CONFIRMED' || status === 'VERIFYING' || status === 'SENDING')) {
    const { data: updated } = await admin
      ...
      .eq('id', deviceId)
      .single();
    return NextResponse.json({ message: 'Normal mode already on or in progress', device: updated });
  }

  const result = await transitionDeviceMode(admin, deviceId, 'NORMAL', user.id);
  ...
  if (!result.ok) {
    return NextResponse.json(
      { error: result.error ?? 'Failed to disable Emergency Mode', device: updated, status: result.status },
      { status: 400 }
    );
  }
